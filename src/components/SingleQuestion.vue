  ...
  <v-card>
    ...
    <template #text>
      ...
      <div v-if="props.question.type !== 'boolean'" class="d-flex ga-5">
        <v-text-field max-width="350px" label="Відповідь" variant="outlined" v-model.trim="answerTitle"
          :error-messages="validateAnswerTitle" />
        <v-btn size="x-large" color="green" @click="handleClickAddOption">Додати відповідь</v-btn>
      </div>
      <v-radio-group v-model="props.question.correctAnswer">
        <div class="d-flex" v-for="option of props.question.options">
          <v-radio class="flex-0-0 mr-2" :value='option' :label="option" color="primary"
